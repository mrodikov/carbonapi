...
func (z Zipper) FetchProtoV3(ctx context.Context, request *protov3.MultiFetchRequest) (*protov3.MultiFetchResponse, *types.Stats, merry.Error) {
	...
	logger.Info("DBG: multiFetchRequest", zap.Any("result", res), zap.Any("stats", stats), zap.Any("err", err))
	if statsSearch != nil {
		if stats == nil {
			stats = statsSearch
		} else {
	...
	if e != nil { // Info
		logger.Info("had errors while fetching result",
			zap.Any("errors", e),
	...
	if res == nil || len(res.Metrics) == 0 {
		logger.Info("no metrics fetched",
			zap.Any("errors", e),
			zap.Any("res", res),
		)
